<?php
/* Footnotes capability based on the 'Simple Footnotes' plugin (http://wordpress.org/extend/plugins/simple-footnotes/) by Andrew Nacin (http://andrewnacin.com/). Plugin has been modified to allow two sets of footnotes and labelling. */
class mcdaniel_footnotes {
        var $footnotes = array();
        var $option_name = 'simple_footnotes';
        var $db_version = 1;
        var $placement = 'content';
        function mcdaniel_footnotes() {
                add_shortcode( 'refa', array( &$this, 'shortcode' ) );
	...
	function shortcode( $atts, $content = null ) {
                ...
                return '<a class="simple-footnote-b" title="' . esc_attr( wp_strip_all_tags( $content ) ) . '" id="return-note-b-' . $id . '-' . $note . '" href="#note-b-' . $id . '-' . $note . '"><sup>' . $note . '</sup></a>';
        }
        ...
        function footnotes( $content ) {
                ...
                if ( empty( $this->footnotes[$id] ) )
                        return $content;
                $content .= '<div class="simple-footnotes-b"><hr><h3>Author Footnotes</h3><ol>';
                foreach ( array_filter( $this->footnotes[$id] ) as $num => $note )
                        $content .= '<li id="note-b-' . $id . '-' . $num . '">' . do_shortcode( $note ) . ' <a href="#return-note-b-' . $id . '-' . $num . '">&#94;</a></li>';
                $content .= '</ol></div>';
                return $content;
        }
}
new mcdaniel_footnotes();
...
class nacin_footnotes {
	...
	function shortcode( $atts, $content = null ) {
        ...
        function footnotes( $content ) {
                ...
                if ( empty( $this->footnotes[$id] ) )
                        return $content;
                $content .= '<div class="simple-footnotes"><hr><h3>Editor Footnotes</h3><ol>';
                foreach ( array_filter( $this->footnotes[$id] ) as $num => $note )
